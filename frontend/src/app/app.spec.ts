...
describe('App', () => {
  ...
  function flushWorkspaceRequests(): void {
    ...
    httpTesting.expectOne('http://localhost:5093/api/admissions/applications').flush([]);
    httpTesting.expectOne('http://localhost:5093/api/admissions/guardians').flush([]);
    ...
    httpTesting.expectOne('http://localhost:5093/api/fees/structures').flush([]);
    httpTesting.expectOne('http://localhost:5093/api/fees/payments').flush([]);
    httpTesting.expectOne('http://localhost:5093/api/fees/concessions').flush([]);
    httpTesting.expectOne('http://localhost:5093/api/fees/receipts').flush([]);
    ...
    httpTesting.expectOne('http://localhost:5093/api/transport/dashboard').flush({
      totalRoutes: 3,
      totalVehicles: 3,
      activeTrips: 2,
      capacityUtilizationPercentage: 67,
      routes: [],
      vehicles: []
    });

    httpTesting.expectOne('http://localhost:5093/api/transport/routes').flush([]);
    httpTesting.expectOne('http://localhost:5093/api/transport/vehicles').flush([]);

    httpTesting.expectOne('http://localhost:5093/api/academic-structure').flush({
      institutions: [
        { id: 1, code: 'TRUST', name: 'Test Education Trust', city: 'Bengaluru', state: 'Karnataka', country: 'India' }
      ],
      campuses: [
        {
          id: 1,
          institutionId: 1,
          institutionName: 'Test Education Trust',
          code: 'TEST',
          name: 'Test Campus',
          city: 'Bengaluru',
          state: 'Karnataka',
